...
public class WithdrawalServiceImpl implements WithdrawalService {
    ...
    @Override
    @Transactional
    public TransactionResponse withdraw(Long walletId, WithdrawalRequest request) {
        Wallet wallet = walletRepository.findById(walletId)
                .orElseThrow(() -> new ResourceNotFoundException("Billetera no encontrada con ID: " + walletId));

        if (!wallet.isActive()) {
            throw new BusinessException("No se pueden realizar retiros en una billetera inactiva o bloqueada");
        }

        BigDecimal previousBalance = wallet.getBalance();

        try {
            // 1. Aplicar débito al saldo de la billetera (Lógica de Dominio valida saldo suficiente)
        ...
        Transaction transaction = Transaction.builder()
                ...
                .reference(request.getReference() != null ? request.getReference() : "Retiro de efectivo")
                .build();
        Transaction savedTransaction = transactionRepository.save(transaction);

        // 3. Registrar Movimiento Contable (Salida)
